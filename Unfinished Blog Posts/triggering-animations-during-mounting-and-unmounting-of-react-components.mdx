...
Now, the component should actually be unmounted upon reaching the "exited" stage. Since we now understand how tracking of state works, we should be able to apply styles during mounting and unmounting. Now let's make AComponent fade in during mounting. We need to change the initial value of **entered** to _true_. We also need to define some default styles and set the different styles during each state of the component. First of all, let us define our default styles:
